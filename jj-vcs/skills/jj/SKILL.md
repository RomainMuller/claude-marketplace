...
name: jj
description: >-
  Load this skill BEFORE running any jj command (jj log, jj diff, jj new,
  jj commit, jj describe, jj split, jj squash, jj edit, jj rebase, jj bookmark,
  jj git push, jj abandon, jj restore, jj status, ...).
---
...
- **Immutable Changes** — all changes that have been pushed to a remote become
  immutable, and can only be modified by passing `--ignore-immutable` to
...
### Handling `.git/index.lock` Errors

If a jj command fails with `Could not acquire lock for index file` /
...
Instead:
1. **Wait 2–3 seconds and retry** the command — the lock is often transient.
2. If it persists, check for competing processes: `lsof .git/index.lock` or
   `ps aux | grep jj`.
3. Only remove the lock (`rm .git/index.lock`) if you are certain no other
   process holds it (e.g. `lsof` returns nothing and the lock file is older
   than a few seconds).
4. If in doubt, ask the user before removing the lock.
